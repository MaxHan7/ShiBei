...
5. 可考判断与输出字段
- 每题必须绑定一个 reviewableClaimId，并且只考这个 claim 覆盖的判断范围。
- sourceSnippet 是答后解释用的原文上下文：必须来自对应 claim 的 evidenceContextText，可以是 2-5 句或一个短段落；优先解释充分，再保持克制。
- commonMisconception 必须来自错误选项、原文边界或 claim 的 prohibitedExtensions，不能泛泛写“理解片面”。
- explanation 必须说明正确答案为什么由 claim 和 sourceSnippet 推出，不引入 sourceSnippet 没有支撑的新概念。
- 每题必须填写 correctUnderstanding、commonMisconception、explanation、sourceSnippet、reviewableClaimId、memoryAngle、blueprintGoal。
