...
test("reviewPathPlan prompt separates chapter summary and unit summaries", () => {
  ...
  assert.match(messages.user, /主线中最值得用户带走的关键理解节点/);
  assert.match(messages.user, /unit 是可复习的独立学习对象/);
  assert.match(messages.user, /判断一个内容是否成为 unit/);
  assert.match(messages.user, /默认不保留背景、铺垫、普通例子、局部事实/);
  assert.match(messages.user, /只因好出题而被选中的点/);
  assert.match(messages.user, /不因覆盖完整而保留低价值 unit/);
  assert.match(messages.user, /也不要因为某段有 source evidence 就自动保留/);
  assert.match(messages.user, /unit 数量由主线价值、用户注意力收益和必要来源证据决定/);
  assert.match(messages.user, /不要使用固定范围控制产量/);
  assert.match(messages.user, /承载主线理解或关键行动判断/);
