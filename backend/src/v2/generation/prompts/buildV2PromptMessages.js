...
function buildTaskBriefPlanMessages({ article, source, blocks, sourceContextNote, plan, unitKnowledgeMap }) {
  return {
    ...
    user: [
      ...
      "- unitKnowledgeMap.microKnowledgePoints 是上游选择出的高价值考察角度，不是覆盖清单。",
      "- 只选择最值得占用用户注意力的考察角度；不要为了覆盖所有 high / medium microKnowledgePoint 而出题。",
      "- 每个 unit 通常只生成 1 个核心 questionPlan；只有当第二个角度明显服务主线且不会显著增加负担时，最多生成 2 个。",
      "- 被跳过的角度不是失败；lean 版允许为了减负而放弃低增益题目。",
      ...
      "- 如果最强 value angle 是清晰的结构、流程、角色、条件、特征或判断依据等对应关系，才生成 matching 计划。",
...
function buildUnitKnowledgeMapMessages({ article, source, blocks, sourceContextNote, plan, compactRetry = false }) {
  return {
    ...
    user: [
      "阶段：unitKnowledgeMap。",
      "任务：为当前输入的 unit 确认最值得考察的价值角度 microKnowledgePoints；本阶段不生成题目、不选择题型、不做 selectedTasks。",
      "为什么需要这一层：",
      "- lean 版不是完整拆解 unit，而是确认哪些角度最值得占用用户注意力。",
      "- 这些价值角度是后续 ECD evidence 和题目生成的上游选择依据，不是覆盖清单。",
      "拆分原则：",
      ...
      "- 价值角度可以体现理解本质、判断场景、区分边界、避免误用、迁移应用或关系理解。",
      "- 优先保留能改善用户理解、判断、行动或避免误解，并且原文中有清晰支撑的点。",
      ...
      "- 对分层模型、结构框架、流程、信号、角色或类型集合，只保留最能帮助用户判断或行动的关系角度。",
      "输出字段：",
      ...
      ...(compactRetry
        ? [
            "重试压缩模式：",
            "- 上一次输出过长或 JSON 没闭合；这次必须保持同样的高价值角度选择，但用更短索引字段。",
...
function buildEcdPlanningMessages({ article, source, blocks, sourceContextNote, plan, unitKnowledgeMap }) {
  return {
    ...
    user: [
      ...
      "- 题目数量由注意力收益和掌握证据价值决定；每个 unit 通常选择 1 个任务，必要时最多 2 个。",
      ...
      "- 如果一个 unit 内含多个目标，优先选择最能改善理解、判断、行动或避错能力的目标；不要把互补角度都变成题。",
      "字段约束：",
      ...
      "- units[].assessableTargets[].coverageRequirement 只能是 required、supporting、optional；required 代表高价值候选，不代表必须出题覆盖。",
      ...
      "- 如跳过未进入 selectedTasks 的 target，可写 skippedTargets；不要输出 skippedEvidence、learningClaims、evidenceNeeds、taskPlan 或 articleUnderstanding。",
      "",
      `reviewPathPlan:\n${JSON.stringify(plan, null, 2)}`,
      "",
      `unitKnowledgeMap:\n${JSON.stringify(unitKnowledgeMap || {}, null, 2)}`,
