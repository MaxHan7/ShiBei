...
function multipleChoiceOptionToneRules() {
  return [
    ...
    "- 选项要短，但不能退化成单词、术语或短标签；每个选项都应是能回答题干的可判断短语。",
...
function buildTaskBriefPlanMessages({ article, source, blocks, sourceContextNote, plan, unitKnowledgeMap, generationLanguage }) {
  return {
    ...
    user: [
      ...
      "- unitKnowledgeMap.microKnowledgePoints 是已筛出的核心 unit 内部的高价值考察点，不是全文覆盖清单。",
      "- 对每个 high microKnowledgePoint 都要形成 practiceGoal 或 questionPlan；medium 如果有独立掌握证据，也应优先覆盖。",
      "- 每个 unit 通常生成 2-3 个 questionPlan；只有当原文存在更多不可合并的独立高价值角度时才超过。",
      "- 不要为了减少题量漏掉同一核心 unit 中独立的高价值角度；也不要为了凑题生成无证据的题。",
      "- 不要把相邻 unit 的内容打包进当前 unit 题目。",
      "- 题型服务于具体考察角度：理解本质、判断场景、区分边界、避免误用、迁移应用或关系理解。",
      "题型选择：",
      ...
      "- 只有当前 source 能支撑至少 2 个稳定对应关系时，才生成 matching；如果只有 1 个关系点，用 multiple_choice 承载该 value angle。",
...
function buildReviewPathPlanMessages({ article, source, blocks, generationLanguage }) {
  return {
    ...
    user: [
      ...
      "移动端长度上限：",
      "- title 不超过 36 字符。",
      "- 如果输出 English，title 必须是完整短语，优先 3-5 个词；不要复制并截断长原文标题。",
      "- summaryCard.text 不超过 96 字符。",
      "- unit.title 不超过 40 字符。",
      "- 如果输出 English，unit.title 也必须是完整短语，不要以 unfinished modifier 结尾。",
      ...
      "- 判断一个内容是否成为 unit 时，依次检查：它是否服务内容主线；是否值得占用用户注意力；是否能改善理解、判断、行动或避错；是否由具体 source blocks 支撑。",
      "- 默认不保留背景、铺垫、普通例子、局部事实、漂亮句子、平台 hook，或只因好出题而被选中的点。",
      "- 不因覆盖完整而保留低价值 unit；宁可少而准，也不要把用户注意力摊开。",
      "- layered_framework、process_steps、type_set、boundary_rule 只有在承载主线理解或关键行动判断时，才应成为 unit。",
      "- 不要把每个段落机械切成 unit；也不要因为某段有 source evidence 就自动保留。",
      "- unit 数量由主线价值、用户注意力收益和必要来源证据决定；不要使用固定范围控制产量。",
      "- 短文可以只有少量 unit；长文或结构密集文章也应先压缩到关键节点。合并发生在多个点服务同一个主线理解时，而不是为了形式覆盖。",
...
function buildUnitKnowledgeMapMessages({ article, source, blocks, sourceContextNote, plan, compactRetry = false, generationLanguage }) {
  return {
    ...
    user: [
      ...
      "- 这些考察点是后续 ECD evidence 和题目生成的上游 inventory，但仍只服务当前核心 unit。",
      "拆分原则：",
      "- 本次输入可能只包含一个 unit；只为输入 plan.units[] 中出现的 unit 输出 units[]。",
      "- 对每个输入的 reviewPathPlan.units[] 都输出一个 units[] 对象。",
      "- microKnowledgePoint 是围绕该核心 unit 展开的高价值考察点，不是低价值细节清单。",
      "- microKnowledgePoints 可以体现定义、本质、边界、模型层级、机制、流程步骤、场景应用、常见误区、关键例子或关系。",
      "- 优先保留能改善用户理解、判断、行动或避免误解，能形成后续可观察理解表现，并且原文中有清晰支撑的点。",
      "- 如果多个表述服务于同一个学习点，合并为一个 micro。",
      "- 每个核心 unit 通常保留 2-3 个 micro；只有不可合并的独立高价值角度才超过。",
      "- assessmentValue 描述该角度是否值得占用用户注意力，不表达题目数量。",
      ...
      "- 对分层模型、结构框架或层级体系，要保留整体结构、关键层级作用、层级关系或边界中有考察价值的点。",
      ...
      ...(compactRetry
        ? [
            "重试压缩模式：",
            "- 上一次输出过长或 JSON 没闭合；这次必须保持同样的核心考察点覆盖，但用更短索引字段。",
...
function buildEcdPlanningMessages({ article, source, blocks, sourceContextNote, plan, unitKnowledgeMap, generationLanguage }) {
  return {
    ...
    user: [
      ...
      "- unitKnowledgeMap.microKnowledgePoints 是已筛出的核心 unit 内部的高价值考察点；不要在本阶段重新压缩它。",
      "- ECD 用来把多个高价值考察点转成可观察掌握证据和合适题型，不是再做一次只留最强角度的筛选。",
      "- 对每个 assessmentValue 为 high 的 microKnowledgePoint，优先保留为 assessableTargets；medium 如果有独立观察价值，也应进入覆盖判断。",
      ...
      "- 每个 unit 通常选择 2-3 个 selectedTasks；只有不可合并的独立高价值角度才超过。",
      "- 不要把相邻 unit 的内容打包进当前 unit selectedTasks。",
      "- matching 应被积极用于天然结构关系：分层模型、类型集合、流程步骤、信号动作、角色职责等关系，如果原文有证据支撑，就是高价值候选。",
      "- 只有当前 source 能支撑至少 2 个稳定对应关系时，才把 selectedTask.taskAffordance 设为 matching；否则使用 multiple_choice。",
