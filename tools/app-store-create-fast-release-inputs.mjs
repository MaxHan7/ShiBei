...
const decisionValues = {
  decisions: {
    ...
    "每日真实 AI 生成额度": args["daily-generation-quota"] || "每天 3 篇，按 UTC day",
    "推荐好文是否计入额度": args["recommended-articles-quota"] || "不计入",
    "匿名用户是否可直接生成": args["anonymous-generation"] || "可以，不强制登录",
    "首版是否加入可选 Apple 登录": args["apple-login"] || "首版暂不做 Apple 登录",
    "如果首版暂不做 Apple 登录，是否接受匿名数据恢复边界": args["anonymous-recovery-boundary"] || "接受，并在说明中明确重装、换机、系统重置可能无法恢复",
    "如果首版做 Apple 登录，是否同步做删除账号入口": args["account-deletion"] || "不适用：首版暂不做 Apple 登录",
    "支持邮箱": supportEmail,
