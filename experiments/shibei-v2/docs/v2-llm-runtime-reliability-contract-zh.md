...
4. 下一步：把 V2 job runner 接入持久化 worker，使用 `retryDelayMs` 更新 `generation_jobs.available_at`。
5. 下一步：对 `empty_structured_text` 增加 stage-specific recovery retry。
6. 下一步：跑同一篇黄金文章，比较 structured-output failure 是否下降。
7. 通过 3-5 篇文章后，再把这套框架沉淀成正式 Codex skill。
