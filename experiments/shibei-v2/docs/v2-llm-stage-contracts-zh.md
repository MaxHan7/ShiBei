...
| `ecdPlanning` | `SingleUnitPlan + MicroKnowledge + UnitSourceWindow -> UnitTaskModel` | 当前 unit window | 题目计划 adapter |
| `unitPracticePlan` | `UnitTaskModel -> PracticePlan` | 不调用模型 | 题目 draft stages |
| `multipleChoiceDraft` | `MCPlans + UnitSourceWindow + ECDContext -> MultipleChoiceDraft` | 当前 unit window | 前端题目 |
| `matchingDraft` | `MatchingPlans + UnitSourceWindow + ECDContext -> MatchingDraft` | 当前 unit window | 前端题目 |
| `unitSummaryDraft` | `Unit + Questions + UnitSourceWindow -> UnitOverviewAndSummary` | 当前 unit window | 前端单元页 |
...
## 5. deterministic `unitPracticePlan` adapter
...
## 9. `qualityDiagnostics` / optional future `qualityJudge`
...
1. `unitKnowledgeMap`、`ecdPlanning`、draft stages 仍有路径会收到全文 blocks，需要 source-context slimming 修复。
2. HTML report 还没有明确展示“本阶段实际传了多少 source blocks”。
3. Stage metrics 还没有系统化记录，只能靠质量报告人工对比。
4. `qualityJudge` prompt 仍存在于代码中，但默认不启用；后续若保留，应改名或迁移为实验性小审查模块，避免误解为主链路。
