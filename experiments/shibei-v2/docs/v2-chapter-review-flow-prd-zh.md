...
匹配/连线题采用即时反馈模式：用户每完成一条连接后立刻看到反馈。匹配正确时，连接线变为绿色并锁定；匹配错误时，连接线短暂变红并自动断开，用户需要重新匹配。只有当该题内所有匹配项都正确完成后，下一步按钮才可点击。这个模式的目的不是制造惩罚，而是让用户在轻交互中快速校准概念边界。

连线题的基础手势可以是：用户按住左侧选项，拖向右侧选项。拖动过程中，一条动态连接线从原选项跟随到用户手指位置；用户在目标选项上松开后，系统判断匹配结果。若匹配正确，连接线和相关选项边框变为绿色并保持连接；若匹配错误，连接线和目标选项短暂变红，选项可以轻微震动，然后连接线自动断开。反馈应明确、轻量，让用户立刻知道这次匹配是否成立。
...
每道题仍然需要后台绑定来源依据，但来源依据不应默认变成解释页里的一小段原文片段。用户真正需要原文帮助的时刻，往往是在答题前毫无思路时。因此，答题页应提供“查看原文”入口，作为主动回源复习的帮助方式。
...
答题后可以优先探索 IP 形象气泡反馈：用户提交答案后仍停留在当前题目页，题目、选项和自己的选择仍然可见；页面侧边的 IP 形象弹出一段类似说话气泡的反馈，告诉用户本题的常见误区或正确理解。用户可以关闭气泡，也可以点击 IP 形象再次唤起气泡。页面底部保留“继续 / 下一题”主按钮，由用户主动推进。

答后反馈气泡是题级反馈，不是知识点级反馈。每一道轻量理解题或场景应用题提交后，都应有机会弹出一次短反馈气泡。样章里可以为了阅读压缩只展示代表性气泡，但产品流程不能理解为每个知识点只有一次反馈。

反馈气泡最终展示“正确理解式解释”还是“常见误区式解释”暂时不定，后续需要二选一验证。两者都可以在后台生成和保留，但不应默认同时展示给用户，因为它们都在承担“帮助用户理解为什么这个答案成立”的任务；同时展示会让阅读体验变重，也可能造成信息重复。第一阶段可以先在样章里保留两类文字用于判断质量，真正进入产品交互时再选择更适合用户理解的一种。
...
16. As a learner, I want to know when a knowledge point is complete, so that I feel momentum.
17. As a learner, I want the chapter to end with a completion summary, so that I know what I took away.
18. As a learner, I want the app to remember where I stopped, so that I can continue later.
19. As a learner, I want the experience to feel relaxed rather than exam-like, so that I am willing to open it often.
20. As a learner, I want the app to adapt in the future when an article type does not fit this flow, so that the review does not feel forced.
21. As a product builder, I want the backend to generate a review path rather than flat questions, so that the frontend can render the intended learning journey.
22. As a product builder, I want prompts to output knowledge units, lightweight understanding checks, scenarios, source anchors, and feedback candidates, so that content matches the V2 flow.
23. As a product builder, I want progress tracked by path step, so that the app can resume precisely.
24. As a product builder, I want clear result data for lightweight understanding checks and scenario applications, so that future personalization can be built.
25. As a product builder, I want the V2 schema isolated from V1, so that we can redesign freely without risking TestFlight production.
...
- Feedback payloads may contain correct-understanding and common-misconception candidates, but the user-facing UI should choose one concise feedback style rather than displaying both by default.
...
- Frontend state tests should verify resume behavior: summary card, current knowledge unit, question card, submitted answer, feedback bubble visibility, scenario, and chapter completion.
