...
- 上传按钮以独立 `upload-tab-button.svg` 作为正式视觉资产。该 SVG 是 `60 x 60` 画布，按钮真实圆心在 `y=26`，阴影占用底部空间；SwiftUI 当前直接使用 `Image("V2UploadTabButton")` 渲染该资产，避免重新手绘导致圆形、阴影或加号位置和组件库不一致。
...
- 中间上传入口当前使用单一视觉状态，并已归档为 `upload-tab-button.svg` 正式资产源。SwiftUI 直接引用该资产；如果后续需要 pressed、loading、success 等状态，再补充对应状态资产或动效说明。
...
- 底部导航：`V2BottomNavigationBar` 已使用 `nav-icons-32frame-source.svg` 拆出的 8 个 `32 x 32` 普通 tab 资产；中间上传按钮已改为独立 `V2UploadTabButton`，直接引用 `upload-tab-button.svg` 对应的 `60 x 60` 资产，不再手绘圆形和加号。
...
- 笔记页：已接入 `notes-mascot.svg`、`notes-bookmark.svg`、`notes-summary-wave.svg`；`V2NotesSummaryCard` 和 `V2SavedQuestionCard` 已能在模拟器中通过底部导航进入和渲染。当前仍需继续精修 summary card 的 IP 遮挡关系、收藏题卡字段间距和底部导航遮挡下方卡片的问题。
...
- 上传页：当前输入框和 IP 分层只是第一版结构，后续需要继续按 Figma 校准输入卡片、placeholder、生成中/失败等状态。
...
- 验证上传页：底部导航进入“上传”，点击“生成复习路径”后进入“章节概要”。
