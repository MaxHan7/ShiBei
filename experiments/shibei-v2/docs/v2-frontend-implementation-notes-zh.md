...
- 中间上传入口当前使用单一视觉状态，并已归档为 `upload-tab-button.svg` 参数源。SwiftUI 不直接依赖 SVG filter；如果后续需要 pressed、loading、success 等状态，再补充对应状态参数或动效说明。
...
- 底部导航：`V2BottomNavigationBar` 已使用 `nav-icons-32frame-source.svg` 拆出的 8 个 `32 x 32` 普通 tab 资产；中间上传按钮已改为独立 `V2UploadTabButton`，按 `upload-tab-button.svg` 参数源原生绘制 `60 x 60` 按钮，不再受 SVG filter 渲染影响。
- 圆形按钮：返回按钮和查看原文按钮已使用 `circle-button-back.svg`、`circle-button-source.svg` 导入后的 asset；通知/个人按钮仍由代码绘制，后续如有独立资产可替换。
- 主页：已接入 `mascot-static.svg`、背景植物装饰和底部导航；路径布局已改为基于 Figma 黄金参考节点 slot 的 SwiftUI 模板，节点不再使用临时百分比坐标，路径会截断在最后一个节点处。
...
- 首页路径曲线：当前 SwiftUI 已使用 `path-cycle-node-placement-reference.svg` 的节点 slot 规则做第一版模板化落地，并保证最后一段停在最后节点；曲线本身仍是 SwiftUI Catmull-Rom 近似曲线，后续若要完全一致，可继续把黄金 SVG path 转为正式 `Path` 周期资产。
