...
- 标签文案：`当前章节`
- 标签颜色：`#98A35E`
- 标签字号实现参考：`11`，行高约 `14`，半粗体
- 章节标题颜色：`#645B51`
- 章节标题字号实现参考：`17`
- 章节标题行高：约 `24`
- 章节标题最多两行，右侧为详情按钮预留空间
- 右侧详情按钮 icon：`24 x 24`，描边色 `#9EA860`，描边宽度 `1.5`

实现共识：

- SwiftUI 中建议抽象为 `CurrentChapterBanner` 组件。
- 文本必须用真实 `Text`，标题支持动态内容和两行换行。
...
- `V2CircleIconButton(kind: .sourceDocument)`：顶部查看原文/来源按钮。
...
- 连线卡片统一尺寸 `140 x 90`、圆角 `15`、文字字号 `14`、行高 `24`，状态变化只切换 fill / border / text color / shadow color。
...
- 上传按钮使用独立 `upload-tab-button.svg` asset。该 SVG 是 `60 x 60` 画布，按钮真实圆心在 `y=26`，阴影占用底部空间；SwiftUI 中直接以 `60 x 60` 原始资产渲染，不再用代码手绘圆和加号，避免细节偏差。
...
- 中间上传入口当前使用单一视觉状态，并已归档为 `upload-tab-button.svg`。如果后续需要 pressed、loading、success 等状态，再补充对应状态资产或动效说明。
...
- 底部导航：`V2BottomNavigationBar` 已使用 `nav-icons-32frame-source.svg` 拆出的 8 个 `32 x 32` 普通 tab 资产；中间上传按钮已改为独立 `V2UploadTabButton` / `upload-tab-button.svg`，不再手绘圆和加号。
- 圆形按钮：返回按钮和查看原文按钮已使用 `circle-button-back.svg`、`circle-button-source.svg` 导入后的 asset；通知/个人按钮仍由代码绘制，后续如有独立资产可替换。
- 主页：已接入 `mascot-static.svg`、背景植物装饰和底部导航；路径曲线仍是第一版 SwiftUI 曲线骨架，尚未完全按黄金路径周期实现。
...
- 上传页：已接入 `upload-mascot-back.svg`、`upload-mascot-front.svg` 和 `upload-link-icon.svg`。
- 通知页：已接入 `notification-mascot.svg`、`notification-success-icon.svg`、`notification-failure-icon.svg`。
- 全部章节页：已接入 `materials-mascot.svg`；章节卡片目前是代码组件骨架。
- 发现页：已接入 `discover-hero-mascot.svg`、`discover-article-thumbnail.svg`；filter chip 和推荐文章卡片是代码组件骨架。
- 笔记页：已接入 `notes-bookmark.svg`；顶部笔记 IP、波形和卡片排版仍需进一步精细化。
...
- 首页路径曲线：当前 SwiftUI 仍是本地骨架曲线，不是 `path-cycle-node-placement-reference.svg` 中的黄金周期路径。后续需要按“起点 + 每个半弧两个知识点节点 + 最后一段截断到最后节点”的规则重写路径布局。
- 路径节点 icon：当前开始节点旗帜和知识点星星仍用 SF Symbols；文档建议后续由 Figma 导出节点 icon 资产，容器、阴影、分段进度环由 SwiftUI 绘制。
...
- 笔记页：顶部 summary card、收藏题目卡、tag 字体和间距仍需按 `notes-summary-card-reference.svg`、`notes-card-reference.svg` 继续细化。
- 个人主页：顶部专属 IP/头像尚未归档为最终资产；当前只避免误用笔记页 IP。
- 圆形通知/个人按钮：目前代码绘制，若后续需要完全匹配 Figma，应补齐独立同画布资产。
...
2. 章节概要继续进入第一个知识点总览页。
3. 知识点总览继续进入本知识点第一题。
4. 题目作答后显示答后反馈，继续进入下一题或单元总结页。
5. 单元总结继续进入下一个知识点总览页；如果已经是最后一个知识点，进入章节总结页。
6. 章节总结可返回首页或进入章节详情。
7. 题目页查看原文会进入原文页，返回后恢复到原题页面。
