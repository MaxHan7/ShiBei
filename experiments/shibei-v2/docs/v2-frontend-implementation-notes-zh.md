...
- 章节详情页：已从只读信息卡升级为当前章节交汇页，包含章节说明、知识点/题量/完成进度、`继续复习`、`查看原文` 和单元状态列表；入口来自首页当前章节 banner、资料页章节卡和章节总结弱入口。
...
10. 章节详情页点击“查看原文”会进入章节级原文页，返回后恢复章节详情页；点击“继续复习”会进入首页当前复习节点对应的单元，如果当前节点不是单元则回退到第一个单元。
...
- 验证章节详情页：首页当前章节 banner 的详情按钮进入“章节详情”；页面显示 `2/3` 进度、`继续复习`、`查看原文` 和单元列表；“查看原文”进入章节级原文页并可返回详情；“继续复习”进入当前复习单元。
