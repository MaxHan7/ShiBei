...
- **覆盖规则**：`required` evidence 不能被跳过；如果某个 required evidence 没有出现在任何 selected task 的 `evidenceIds[]` 中，`ecdPlanning` validation 会失败。
...
- **前端使用位置**：默认不进入正式 UI；可在 debug 或质量报告中查看。
- **展示规则**：不展示给普通用户。
- **注意事项**：这是过渡 adapter 字段。它应消费 ECD `selectedTasks[]`，而不是重新独立选择题型。后续应逐步被 ECD 字段组替代：`unitLearningClaims[]`、`unitEvidenceNeeds[]`、`unitTaskPlan[]`、`unitAssemblyPlan[]`。不要继续把“每个 unit 固定 2 道题”写进该字段的语义。
