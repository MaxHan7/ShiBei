...
function buildUnitKnowledgeMapMessages({ article, source, blocks, sourceContextNote, plan }) {
  return {
    ...
    user: [
      ...
      "- summary 是一句短说明，说明这个小点是什么；不要粘贴长原文。",
      "- role 只能使用 schema enum。",
      "- suggestedEvidenceAngles 只写 1-3 个建议观察角度，不选择题型；常见映射：definition -> definition_grasp，boundary -> boundary_discrimination，model_layer/relationship -> structure_mapping，mechanism -> mechanism_reasoning，process_step -> step_purpose_mapping，scenario_application -> scenario_transfer，misconception -> misconception_detection。",
