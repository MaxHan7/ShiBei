...
function buildUnitCopyBatchMessages({ article, source, units }) {
  return {
    ...
    user: [
      ...
      "- 可以参考该 unit 的 sourceContext、practiceGoals 和 questionPlans 来把握重点，但不要输出题目、题干、选项或答案。",
...
function buildTaskBriefPlanMessages({ article, source, blocks, sourceContextNote, plan, unitKnowledgeMap }) {
  return {
    ...
    user: [
      ...
      "- 你要自然完成这条链路：learning target -> observable evidence -> practiceGoal -> questionPlan。",
      "- 对每个 unit 先判断：用户需要掌握什么；什么表现能证明掌握；哪些 micro 应合并成同一个可观察目标；哪种任务最能观察这个 evidence。",
      ...
      "- 每个 high / medium microKnowledgePoint 都应进入覆盖判断；如果不直接形成 questionPlan，也要被合并进相关 practiceGoal；context_only 可以不出题。",
      "- 一个 unit 可以有多个题目计划，数量由掌握证据和考察角度自然决定，不写死题目数。",
      "- 多角度 evidence coverage：对 high-value practiceGoal，优先寻找互补的 evidence angles。",
      "- 互补角度包括：核心理解、边界辨析、误区识别、场景迁移、关系映射。",
      "- 当不同角度能观察到不同掌握表现时，可以为同一个 practiceGoal 设计多个 questionPlans。",
      "- 多个 questionPlans 应分别服务于不同 evidence angle。",
      "- 不要为了减少题量漏掉独立的小知识点；题目数量由 evidence angles 自然决定。",
      "题型选择：",
      "- multiple_choice 适合核心理解、边界判断、误区识别、场景迁移。",
      "- matching 适合需要用户建立多个元素之间稳定对应关系的 evidence。",
...
function buildUnitKnowledgeMapMessages({ article, source, blocks, sourceContextNote, plan }) {
  return {
    ...
    user: [
      ...
      "- suggestedEvidenceAngles 只写建议观察角度，不选择题型；常见映射：definition -> definition_grasp，boundary -> boundary_discrimination，model_layer/relationship -> structure_mapping，mechanism -> mechanism_reasoning，process_step -> step_purpose_mapping，scenario_application -> scenario_transfer，misconception -> misconception_detection。",
      "- sourceAnchorId 必须引用当前 unit.sourceAnchor.id。",
      "- sourceSupport 写该 source 如何支撑这个 micro，不粘贴长原文；它应该能解释为什么该 micro 不是模型臆造。",
      "",
      `reviewPathPlan:\n${JSON.stringify(plan, null, 2)}`,
      "",
      renderSource(source, blocks, sourceContextNote),
