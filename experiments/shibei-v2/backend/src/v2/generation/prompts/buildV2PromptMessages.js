...
function buildEcdPlanningMessages({ article, source, blocks, plan }) {
  return {
    ...
    user: [
      ...
      "- unitAssemblyPlan 要形成 coverage matrix：selectedTasks 覆盖所有 required evidence；题目数量不写死，由 required evidence 覆盖自然决定。",
      ...
      "- unitSubObjectives、unitLearningClaims、unitEvidenceNeeds、unitTaskPlan、unitAssemblyPlan 都必须使用稳定 id，且引用关系必须闭合。",
      "- unitLearningClaims[].subObjectiveId 和 unitEvidenceNeeds[].subObjectiveId 必须引用 unitSubObjectives。",
