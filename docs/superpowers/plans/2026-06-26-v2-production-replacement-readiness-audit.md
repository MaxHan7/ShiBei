...
V2 已完成第一阶段真实入口：笔记页会使用真实 favorite 记录生成 V2 favorite display item，并支持打开收藏题、返回笔记页、继续下一条收藏题。

仍需补齐：

- 题目页右上角收藏/取消收藏按钮要写入真实 favorite API，而不是只保存在本地交互状态。
...
1. 补齐 V2 题目页真实收藏/取消收藏写入。
2. 跑一次 root backend + root iOS V2 Debug 的本地完整生成测试。
3. 修复生成/恢复流程里的状态漏洞。
4. 翻转 Release 入口到 V2。
