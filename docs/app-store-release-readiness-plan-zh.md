...
- [ ] 生成失败率监控：按失败类型统计。当前后端有失败类型和质量实验指标，且 `docs/app-store-production-ops-runbook-zh.md` 已定义人工排查和分级；仍缺 production dashboard/自动告警闭环。
- [ ] APNs 失败监控：BadDeviceToken、BadEnvironmentKeyInToken、未配置等聚合。当前 health audit 检查 APNs configured/environment，通知发送链路记录错误，且 runbook 已定义定位流程；仍缺失败原因自动聚合和告警。
...
| 2026-07-03 | 增加 App Store 首版生产运维 runbook | 已新增最小生产运维文档，覆盖用户影响型 SLI、Railway/API 事故响应、队列/生成失败分级、APNs 错误定位、备份恢复核对、推荐好文回退排查和事故证据模板；自动 dashboard/告警聚合和恢复演练仍作为后续开放项 | `docs/app-store-production-ops-runbook-zh.md`、`docs/app-store-release-evidence/2026-07-03-production-ops-runbook.md` | 上架前按 runbook 做一次恢复演练或明确豁免；继续补用户决策/URL/截图/真机验收 |
