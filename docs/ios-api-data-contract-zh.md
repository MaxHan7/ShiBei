...
`confidenceReasons` 记录低置信原因，例如 `weak_source_support`、`weak_explanation_faithfulness`、`weak_context_relevance`、`question_type_mismatch`、`judge_rewrite`。`blockingReasons` 记录不可入池原因，例如 `answer_not_unique`、`structure_invalid`、`source_not_found`。
...
来源片段 v4-v18 字段用于质量工作台和后续调试，iOS 用户侧第一版可以忽略：
...
- `sourceMinimalityScore`：来源是否克制的辅助机器评分，1-5 分；它不再是主要优化目标。
- `sourceExplanatoryCoverageScore`：来源是否覆盖题干、正确答案和解释里的关键判断，1-5 分。
- `claimCoverageScore`：题目是否围绕绑定的可考判断展开，1-5 分。
- `misconceptionGroundingScore`：常见误区是否能对应错误选项、原文边界或禁止扩展范围，1-5 分。
- `explanationAnswerBindingScore`：解释是否明确连接正确答案、可考判断和来源上下文，1-5 分。
- `sourceOverlapGroupId` / `sourceOverlapRatio`：用于诊断多题是否复用同一语义大段来源。

v18 后端可在 `KnowledgePoint.reviewableClaims` 中返回可选的内部出题锚点。每个 claim 表示“这个知识点里可被考察的一条判断”，包含：

- `id`
- `claim`
- `evidenceText`
- `evidenceContextText`
- `allowedQuestionScope`
- `prohibitedExtensions`
- `memoryAngle`

题目可通过 `reviewableClaimId` 绑定到其中一个 claim。iOS 正式页面不展示这些字段，但解码必须兼容缺失和未知对象。
...
- `sourceSnippet` 是 iOS 解释页用户可见的原文解释上下文。后端以题目绑定的 `reviewableClaim`、知识点 `sourceQuote`、题干、正确理解、常见误区和知识点主张为线索，在 `cleanedText` 中定位最能解释本题的原文句子窗口。
- 后端选择 `sourceSnippet` 时优先保证解释充分：它应覆盖题干、正确答案和解释里的关键判断。通常是 2-5 句或一个短段落；如果更短片段不能解释清楚，允许扩展相邻句，并通过 `sourceContextSelection` 记录扩展原因。完整上下文由“查看完整来源”页面承担。
