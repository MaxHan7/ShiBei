...
- v12 最新 run 保留 7 个知识点、18 道题，动态预期题数 21，道题覆盖率为 85.7%。
- 4 / 7 个知识点达到完整动态覆盖，3 个知识点为部分覆盖；缺失动作主要是 `core_understanding` 2 次、`scenario_application` 1 次。
- 平均每知识点 2.6 道题，4 个知识点达到 3 道题。
- 低置信题为 16 道，低置信比例 88.9%；这说明题量召回有效，但新增题的教学质量还不稳定。
- 平均来源精准度达到 4.9，平均来源最小化达到 4.9，来源质量没有因召回明显恶化。
- 重复练习风险题从 v11 的 0 回升到 6，说明放松召回会重新带来“换壳练习”风险。
- 主线节点从 v9 的 24 个伪结构节点压缩到 13 个，且关键知识点能绑定到对应主线。

这说明：**题量缺口可以通过召回和补题补上，但召回本身不会自动带来教学质量。当前主要矛盾转移到认知动作驱动的质量校准：哪些可恢复题应该 rewrite，哪些重复题应该继续挡掉，核心理解/边界辨析/场景迁移各自应该如何评分。**

因此当前最近主线调整为：**动态覆盖率 + 可恢复题 rewrite + 认知动作级评分闭环**。后续不再把“更多题”当作质量提升，也不把全文章固定题数当作硬门槛，而是把“每个知识点是否按自身价值形成足够的核心回忆、边界辨析、场景迁移练习，并且每道题是否真的完成对应学习动作”作为第一验收口径。

## 下一阶段：v13 可恢复题 rewrite 与认知动作质量校准

v12 证明动态覆盖率口径是必要的，也证明单纯放松召回会带回低置信和重复风险。下一阶段不应继续追题量，而应把 `recoverableBlockedCount` 和低置信题作为 rewrite 对象，按认知动作分别修。

### v13 核心目标

- 保持动态覆盖率，不再追求全篇固定题数。
- 对可恢复候选题做 rewrite，而不是直接低置信入池。
- 对 `core_claim_too_literal`、`boundary_confusion_not_real`、`scenario_is_restatement` 分别使用不同修复指令。
- 重复风险题必须改题干、改场景变量、改干扰项或改来源证据；不能只因为缺题就放行。
- 验收重点是：动态覆盖率不下降，低置信比例下降，重复练习风险下降，认知动作匹配回升。

### v13 优先修复项

1. **核心理解题**：避免只问原文说了什么，改成要求用户主动取回核心判断和因果关系。
2. **边界辨析题**：必须先确定真实混淆对象，再生成干扰项；没有真实混淆对象的题不能伪装成边界题。
3. **场景迁移题**：必须加入原文之外的新情境变量，答案仍由原文原则推出；原文换壳题应 rewrite。
4. **重复风险题**：同一知识点多题如果考同一判断，即使题型或来源不同，也应降级或重写。
