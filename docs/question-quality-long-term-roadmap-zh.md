...
出题系统的最高验收标准单独沉淀在 [拾贝出题系统 PRD 验收标准](./question-generation-prd-acceptance-zh.md)。

后续每一轮实验都必须先说明它对应哪一条 PRD 验收标准：

- 核心知识点有没有漏。
- 题目是否真的帮助用户理解和回忆。
- 解释和来源是否让用户相信系统。
- 题卡是否足够轻，适合随时复习。
- 评分指标是否能定位真实问题，而不是制造噪音。

如果一轮实验只让题量、低置信比例或某个机器分数变漂亮，但不能解释用户体验为什么更好，不应视为成功。

## 当前阶段判断

截至 2026-06-01，出题系统已经从“题量不足、知识点无题”推进到 **教学质量稳定化阶段**。题量覆盖、来源定位和结构诊断已经基本可用；当前核心问题不是继续增加题，而是确认每道题是否真正完成一个学习动作，并且答后来源能充分解释这道题。

### 2026-06-01 v18 最新结论：可考判断层 + 解释充分来源

v18 把生产出题链路从“知识点直接出题”改为“知识点 -> 可考判断 -> 题目”。每个知识点先形成 1-3 个 `reviewableClaim`，题目必须绑定其中一个 claim，再围绕这个 claim 生成题干、选项、误区、解释和来源上下文。

这轮调整同时修正了来源策略：`sourceSnippet` 不再以“越短越好”的最小证据为主目标，而是改成 **解释充分上下文**。来源可以是 2-5 句或一个短段落，只要它能覆盖题干、正确答案和解释里的关键判断；`sourceMinimalityScore` 降级为辅助指标。

固定单篇基准 `UMr6ia1QubqOMw3aBUGbOw` 的 v18 结果：

- 保留知识点 8 个，入池题 18 道，未覆盖知识点 0 个。
- 动态覆盖率 85.7%，4 个知识点达到 3 题，所有知识点至少有 1 题。
- 高置信题比例从 v17 的 20% 提升到 33.3%；需重写题比例从 53.3% 降到 38.9%。
- `answer_not_unique` 阻断从 4 降到 1。
- 平均低摩擦题卡分保持 5，高摩擦题 0。
- 来源精准 4.7、来源解释覆盖 4.4、解释答案绑定 4.4，说明来源稍长但整体仍克制。

v18 的有效性在于：它减少了“知识点自由扩展导致题目、来源、解释互相脱节”的问题，并把未覆盖知识点降为 0。但它也暴露了下一层瓶颈：

- `source_coverage_incomplete` 仍有 5 次，说明 claim 层还不够原子化，部分题仍覆盖了比 claim 更宽的判断。
- `answer_grounding_weak` 仍有 5 次，说明答案与来源的支撑关系仍需校准。
- `boundary_confusion_not_real` 仍有 4 次，边界/误区题还缺少真实混淆对象。
- 重复练习风险为 5，需要继续约束同一知识点内题目是否真的训练不同判断。

下一阶段优先级：不再回到“追更多题”或“追更短来源”，而是继续打磨 **claim 原子化、边界误区真实化、解释与答案绑定**。
...
| 2 | 修来源可信度 | source_precision、source_explanatory_coverage、overlap 诊断 | 解释页来源能充分覆盖题目和解释 |
